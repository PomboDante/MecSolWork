...
Original file is located at
    https://colab.research.google.com/drive/1CsonpYOLZjlrURzK0rTccxD5oBn0Rku7
"""
...
def
solver(n_nos, n_el, coord, conect, prop, restr, forca):
    ndof = 2 * n_nos
...
def criar_modelo_interativo():
    """Guia o usuário para instanciar e popular um objeto TrelicaPlana."""
    modelo = TrelicaPlana()
    ...
    modulo_padrao = input_valor(clr("  Módulo de elasticidade (kPa/MPa/GPa): ", "warn"), float)
    ...
    for i in range(n_el):
        ...
        # Você pode implementar a lógica de perguntar se quer usar a área/módulo padrão
        # ou inserir específicos para cada barra. Aqui usaremos o padrão para simplificar:
...
def main():
    ...
    # Pergunta se o usuário quer digitar ou usar um teste rápido
    resp = input(clr("  Deseja digitar os dados manualmente? (s/n): ", "warn")).lower()

    if resp == 's':
        modelo = criar_modelo_interativo()
    else:
        print(clr("  Carregando dados de exemplo...", "dim"))
        modelo = TrelicaPlana()
